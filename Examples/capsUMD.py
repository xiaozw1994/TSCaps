...
if __name__ == "__main__":
    ...
    print(acc)
    decode = sess.run(caps.decode,feed_dict={caps.x:batch_x, caps.y:batch_y,caps.is_train:False})
    ori = batch_x.reshape((batch_size,length))
    decode=(decode - decode.mean(axis=0))/(decode.std(axis=0))
    z = [i for i in range(1,length+1 )]
    fig = plt.figure()
    aix = fig.subplots(nrows=2,ncols=1)
    aix[0].plot(z,ori[3])
    aix[1].plot(z,decode[3])
    plt.show()
    '''
