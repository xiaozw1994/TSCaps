...
if __name__ == "__main__":
    ...
    for i in range(epoch):
        for b in range(total_train_epoch):
            ...
            e = np.random.randint(0,high=1)
            batch_x = x_train[b*batch_size+e:(b+1)*batch_size+e,:,:,:]
            batch_y = Y_train[b*batch_size+e:(b+1)*batch_size+e,:]
        ...
        if i % 1 == 0:
            ...
            for j in range(total_test_epoch):
                ...
                e = np.random.randint(0,high=29)
                batch_x = x_test[j*batch_size+e:(j+1)*batch_size+e,:,:,:]
                batch_y = Y_test[j*batch_size+e:(j+1)*batch_size+e,:]
                pt += sess.run(caps.acc_num,feed_dict={caps.x:batch_x, caps.y:batch_y,caps.is_train:False}) 
            acc = pt / total_test_epoch / batch_size
