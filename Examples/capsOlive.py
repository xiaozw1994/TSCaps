...
class CapMotion(object):
    def __init__(self,batch_size,length,num_label,decay,is_train=True ):
        ...
        self.length = length
        if self.length % 2 == 0:
            self.next_length = self.length // 2
        else :
            self.next_length = self.length // 2 + 1
        ...
        self.optimizer = tf.train.AdamOptimizer(learning_rate=0.0001,beta1=0.9,epsilon=1e-6)
        self.RMadm = tf.train.RMSPropOptimizer(learning_rate=0.0001)
    ...
    def biuld_net(self):
       ...
       # with gragh.as_default():
                 ...
                 self.primary_cap = layers_vector(top_con,32,4,[9,1],1,self.is_train,shapes=[-1,self.next_length*8,16,1])
                 # [-1,88*16,8,1]
                 ...
                 self.primary_cap_1 = layers_vector(top_conv_1,32,4,[7,1],1,self.is_train,shapes=[-1,self.next_length*16,8,1])
                 ...
                 self.primary_cap_2 = layers_vector(top_con_2,32,4,[5,1],1,self.is_train,shapes=[-1,self.next_length*32,4,1])
...
if __name__ == "__main__":
    ...
    pt = sess.run(caps.acc_num,feed_dict={caps.x:x_test, caps.y:Y_test,caps.is_train:False}) 
    acc = pt /  batch_size
    print("spend: %.3f"%(time.time()-start_time))
    print(acc)
    '''
    for i in range(epoch):
        ...
        _ , local_loss, margin_loss, total_loss = sess.run([caps.train_op,caps.local_loss,caps.margin_loss,caps.total_loss],feed_dict={caps.x:x_train, caps.y:Y_train,caps.is_train:True })
        if i % 2 == 0:
            #pt = 0
