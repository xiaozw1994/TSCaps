...
class CapMotion(object):
    def __init__(self,batch_size,length,num_label,decay,is_train=True ):
        ...
        self.length = length
        if self.length % 2 == 0:
            self.next_length = self.length // 4
        else :
    ...
    def biuld_net(self):
       ...
       # with gragh.as_default():
                ...
                # decoder
                 ...
                 self.decode = tf.contrib.layers.fully_connected(fc1,num_outputs=self.length,activation_fn=None)
...
if __name__ == "__main__":
    ...
    decodes = np.array(np.zeros((batch_size*total_test_epoch,length)))
    predict_arg = np.array(np.zeros(batch_size*total_test_epoch))
    #saver.restore(sess,"data/"+class_name+".ckpt")
    pt = 0
    for j in range(total_test_epoch):
        ...
        predict_arg[j*batch_size:(j+1)*batch_size] = sess.run(caps.argmax_idx,feed_dict={caps.x:batch_x, caps.y:batch_y,caps.is_train:False})   
        decodes[j*batch_size:(j+1)*batch_size,:] = sess.run(caps.decode,feed_dict={caps.x:batch_x, caps.y:batch_y,caps.is_train:False})
    acc = pt / total_test_epoch / batch_size
    print("Now :",acc)
    ori = x_test.reshape((-1,length))
    decodes=(decodes - decodes.mean(axis=0))/(decodes.std(axis=0))
    arg_idx = np.argmax(Y_test,axis=1)
    z = [i for i in range(1,length+1)]
    a1 = np.mean( ori[np.where(arg_idx==0)],axis=0)
    a2 = np.mean(ori[np.where(arg_idx==1)],axis=0)
    a3 = np.mean(ori[np.where(arg_idx==4)],axis=0)
    d1 = np.mean(decodes[np.where(predict_arg==0)],axis=0)
    d2 = np.mean(decodes[np.where(predict_arg==1)],axis=0)
    d3 = np.mean(decodes[np.where(predict_arg==4)],axis=0)
    fig = plt.figure()
    axis = fig.subplots(nrows=2,ncols=3)
    axis[0][0].plot(z,a1)
    axis[0][0].set_title("Original A1")
    axis[0][1].plot(z,a2)
    axis[0][1].set_title("Original A2")
    axis[0][2].plot(z,a3)
    axis[0][2].set_title("Original A5")
    axis[1][0].plot(z,d1)
    axis[1][0].set_title("Decode A1")
    axis[1][1].plot(z,d2)
    axis[1][1].set_title("Decode A2")
    axis[1][2].plot(z,d3)
    axis[1][2].set_title("Decode A5")
    plt.show()
    '''
    h = []
    for i in range(epoch):
        ...
        if i % 1 == 0:
            ...
            #s = np.array(h)
            #np.save("test.npz",s)
            #wirter.add_summary(a)
            #wirter.add_summary(b)
            #wirter.add_summary(c)
            
            if acc >= sota and acc > value:
                value = acc
                saver.save(sess,"data/"+class_name+".ckpt")
                print("Epoch:%d======loss:%.4f=========accuracy:%.5f"%(i,total_loss,acc))
